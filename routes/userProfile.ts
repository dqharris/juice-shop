    ...
    fs.readFile('views/userProfile.pug', function (err, buf) {
      ...
      if (loggedInUser) {
        UserModel.findByPk(loggedInUser.data.id).then((user: UserModel | null) => {
          ...
          // Always HTML-encode the username before embedding it in the template
          ...
          challengeUtils.solveIf(challenges.usernameXssChallenge, () => { return user?.profileImage.match(/;[ ]*script-src(.)*'unsafe-inline'/g) !== null && utils.contains(safeUsername, '<script>alert(`xss`)</script>') })
